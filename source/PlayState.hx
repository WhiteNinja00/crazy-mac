...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var camPos:FlxPoint = new FlxPoint((boyfriendCameraOffset[0] + opponentCameraOffset[0]) / 2, (boyfriendCameraOffset[1] + opponentCameraOffset[1]) / 2);
		camPos.x += ((boyfriend.getGraphicMidpoint().x + boyfriend.cameraPosition[0]) + (dad.getGraphicMidpoint().x + dad.cameraPosition[0])) / 2;
		camPos.y += ((boyfriend.getGraphicMidpoint().y + boyfriend.cameraPosition[1]) + (dad.getGraphicMidpoint().y + dad.cameraPosition[1])) / 2;
	...
	public function startVideo(name:String, endcut:Bool = false)
	{
		#if VIDEOS_ALLOWED
		inCutscene = true;
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					var epicval = 0;
					var coolweekname = WeekData.weeksList[storyWeek].toLowerCase();
					if(Highscore.floorDecimal(ratingPercent * 100, 2) > 75) {
						epicval = 2; //good
						startVideo('goodendingweek1', true);
					} else {
						epicval = 1; //bad
						startVideo('badendingweek1', true);
					}
					crazysave.data.week1 = epicval;
